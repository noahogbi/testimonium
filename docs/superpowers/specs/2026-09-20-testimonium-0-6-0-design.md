...
| value-internal whitespace | **trimmed** before comparison | a deliberate WIDENING: `name=" description "` harvests in 0.6.0 and did not in 0.5.0. More permissive than a browser too - HTML5 matches metadata names exactly - so this is not "what a browser does". It moves text in the ADDING direction, which is the direction that can turn `unreachable` into `unsupported`, so it is disclosed here, in the changelog and in the README |
