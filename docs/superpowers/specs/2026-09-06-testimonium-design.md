...
| **Amended 2026-09-12, testimonium 0.2.0 section 5.3:** `norm` and `defaultFetcher` are now runtime exports from `src/index.ts` - `norm` as a documented compatibility surface, `defaultFetcher` so a caller can wrap or compose the bundled ladder; `isPdf` and the UA and host-rule machinery stay genuinely sealed | 5.1, 5.3 |
| **Amended 2026-09-13, testimonium 0.3.0 section 5.3:** `toText` is now a runtime export from `src/index.ts` as well, joining `norm` and `defaultFetcher`; only `isPdf` and the UA and host-rule machinery remain from the original list | 5.1, 5.3 |
