...
| --- | --- | --- |
| **6.6** the ladder escalates until a read is readable | base spec 6.6; `src/fetch/read-source.ts:87`; `src/classify/verdict.ts:85-86` | it may also escalate past a readable read when the verdict would be `unsupported` and a rung is untried (section 4) |
