...
| **5.3** "The primitives (`norm`, `toText`, `isPdf`, per-host helpers) are genuinely unavailable to consumers" | base spec line 260 | `norm`, `defaultFetcher`, and `toText` become available; `isPdf` and the per-host rule machinery remain unavailable (section 6) |
...
`recordingFetcher` is NOT exported - nor is `isPdf` - and
`CitationResult` carries excerpts, never the source text. A reader would have
reached for an import that does not exist.

What IS available: `defaultFetcher`, `norm`, and `toText` are exported, so a
caller can write its own `Fetcher` wrapper keeping the bytes each rung
returned, normalize with the same `norm` the matcher uses, and extract prose
with the same `toText` the classifier uses. **Amended 2026-09-13, testimonium
0.3.0:** the originality caller uses `toText` directly; the second-extractor
question this paragraph used to raise is closed.
