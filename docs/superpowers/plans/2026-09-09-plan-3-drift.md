...
- **No-baseline is evaluated before the named confounds**, although 8.3's procedure lists the confound row first. Every confound predicate reads a field off the baseline entry (`claimsHash`, the recorded `pdftotextVersion`, `localRulesHash`), so with no entry there is nothing to compare. This is an ordering forced by the data, not a disagreement with the ruling.
...
- **A named confound short-circuits the procedure, exactly as 8.3 says - but `liveGone` is reported anyway.** 8.3 puts the confound row above the `documentGone` row, so an author who edited a claim would, under a literal reading, never be told their source 404s. This plan follows 8.3's ordering for the CATEGORY and the exit contribution (confounded, 0), and additionally carries a `liveGone` boolean on every outcome which the report prints as a clause. Nothing about the exit code changes. **I believe 8.3 is arguably wrong here** - a named confound explains a difference between `A` and `R`, and gone-ness is a fact about `L` that no confound can explain - and that argument is recorded in the draft report rather than applied to the spec.
...
---
### Task 1: Preserve Fable's review of 8.3 into the repository
...
**Interfaces:**
- Consumes: nothing.
- Produces: a committed path that every later task, the ledger and the spec may cite. No code.

**Why this task is first.** 8.3 already cites this review by name ("Fable's review of this section returned APPROVE WITH CORRECTIONS with fourteen findings"), and every later task in this plan argues from its findings. `.superpowers/sdd/` is git-ignored - `.superpowers/sdd/.gitignore` is a single `*` - so a path citation added before the copy exists would be a dangling reference in a committed document. Plan 2 shipped **27 citations pointing at git-ignored scratch** across 16 source and test files, and had to preserve two review documents in its final fix wave to make them resolve. Do it before anything else cites it by path. The precedent, including the wording of the header block, is `docs/superpowers/plans/2026-09-08-plan-2-harvest-fable-plan-review.md`.

- [ ] **Step 1: Copy the review into the repository**
...
wc -l docs/superpowers/plans/2026-09-09-plan-3-drift-fable-spec-review.md
```

Expected: `252 docs/superpowers/plans/2026-09-09-plan-3-drift-fable-spec-review.md`.

- [ ] **Step 2: Prepend the header block that names the numbering this document owns**

Insert these six lines at the very top of the copy, above its existing `# Fable spec review: section 8.3 ...` heading, with one blank line after them. Use the Write or Edit tool, not a heredoc.
...
- [ ] **Step 3: Verify the copy is byte-identical below the header, ASCII, and NUL-free**

```bash
cd C:/Users/noaho/testimonium-plan3 && \
printf 'a\0b' | LC_ALL=C tr -cd '\000' | wc -c && \
printf 'caf\303\251' | LC_ALL=C tr -d '\000-\177' | wc -c && \
tail -n +8 docs/superpowers/plans/2026-09-09-plan-3-drift-fable-spec-review.md \
  | diff - .superpowers/sdd/2026-09-09-plan-3-drift/fable-spec-review.md && echo "IDENTICAL BELOW HEADER" && \
LC_ALL=C tr -d '\000-\177' < docs/superpowers/plans/2026-09-09-plan-3-drift-fable-spec-review.md | wc -c && \
LC_ALL=C tr -cd '\000' < docs/superpowers/plans/2026-09-09-plan-3-drift-fable-spec-review.md | wc -c
```

Expected, in order: `1` (the NUL instrument can fail), `2` (the non-ASCII instrument can fail), `IDENTICAL BELOW HEADER`, `0`, `0`. If `diff` prints anything, the copy was altered - restore it and re-prepend. If the first two numbers are not `1` and `2`, the instruments are broken and every later `0` in this plan is worthless: fix them before continuing.
...
```bash
cd C:/Users/noaho/testimonium-plan3 && \
git add docs/superpowers/plans/2026-09-09-plan-3-drift-fable-spec-review.md docs/superpowers/specs/2026-09-06-testimonium-design.md && \
git status --short && \
git -c core.safecrlf=false commit -m "docs(plans): preserve Fable's review of spec 8.3 out of git-ignored scratch"
```

`git status --short` must show exactly the two paths staged and nothing else.
...
describe("pdftotextVersion", () => {
  ...
  it("does not treat a non-zero exit as absence - Xpdf exits 99 on -v and IS installed", () => {
    // `missing` is set from spawnSync's `error`, never from its `status`.
    // pdftotextAvailable already draws this line (only ENOENT means absent);
    // a probe that drew it differently would advertise the rung and record no
    // version for it.
    expect(pdftotextVersion(() => probe({ missing: false, stderr: XPDF_STDERR }))).toBe("pdftotext version 4.00");
  });
...
Expected: `19 passed` (12 existing plus 7 new), then `TSC CLEAN`.
...
Expected: FAIL naming "reads the version off stderr", "does not treat a non-zero exit as absence" and "prefers stderr"; then `19 passed` after the restore.
...
node -e "import('./dist/fetch/pdf.js').then(m => console.log(JSON.stringify(m.pdftotextVersion())))"
```

Expected on a machine with `pdftotext`: a JSON string, e.g. `"pdftotext version 4.00"`. On a machine without it: `null`. **Record what it printed, and the machine, in the ledger.** Nothing in the suite depends on this value.
...
Expected: `1`, `2`, four `0`s, the previous total **plus 7**, `TSC CLEAN`.
...
const read = (rung: RungId, rawBody: string, status = 200): { rung: RungId; response: RawResponse } => ({
  rung,
  response: { rawBody, status, headers: { "content-type": "text/html" }, finalUrl: "", bytes: rawBody.length },
});
...
function staged(bodies: string[], archivedAt = "2026-09-09T00:00:00.000Z"): StagedEntry {
  return buildArchiveEntry({
    verdict: "supported",
    claims: ["spending rose sharply"],
    reads: bodies.map((b, i) => read(i === 0 ? "node" : "curl", b)),
    toolVersion: "0.1.0",
    localRulesHash: null,
    pdftotextVersion: null,
    archivedAt,
...
describe("the archive store", () => {
  ...
  it("is idempotent: re-archiving unchanged bytes rewrites the same hash and stores no new blob", () => {
    const dir = tmp();
...
import { ARCHIVE_VERSION, blobRelPath, type ArchiveEntry, type ArchiveIndex } from "./format.js";
...
Expected: FAIL naming "MERGES: an entry for another URL survives a later write"; then `13 passed`.

- [ ] **Step 6: Confirm the temp directories were cleaned up**

```bash
cd C:/Users/noaho/testimonium-plan3 && git status --short && echo "TREE CLEAN ABOVE (only the two new files should appear)" && \
ls "$TMPDIR" 2>/dev/null | grep -c "testimonium-archive-" || echo "0 leftover temp dirs"
```

Expected: `git status --short` shows only `?? src/archive/store.ts` and `?? test/archive/store.test.ts`; no `testimonium-archive-*` directories survive.
...
```bash
cd C:/Users/noaho/testimonium-plan3 && npm run build >/dev/null 2>&1 && \
rm -rf /tmp/tm-e2e && mkdir -p /tmp/tm-e2e && \
node -e '
...
http.createServer((q, s) => { s.writeHead(200, {"content-type": "text/html", "set-cookie": "session=SECRET123"}); s.end(body); })
  .listen(8765, "127.0.0.1", () => console.log("up"));
' & \
sleep 2 && \
printf 'The claim.[^1]\n\n[^1]: The Committee Report, http://127.0.0.1:8765/report\n' > /tmp/tm-e2e/doc.md && \
printf '{"http://127.0.0.1:8765/report": ["spending rose sharply"]}\n' > /tmp/tm-e2e/doc.claims.json && \
node dist/bin.js check /tmp/tm-e2e/doc.md; echo "EXIT=$?"
```
...
```bash
cd C:/Users/noaho/testimonium-plan3 && \
find /tmp/tm-e2e/doc.archive -type f | sort && \
echo "--- index ---" && cat /tmp/tm-e2e/doc.archive/index.json && \
echo "--- set-cookie must NOT appear ---" && \
grep -ci "set-cookie\|SECRET123" /tmp/tm-e2e/doc.archive/index.json || echo "0 (good)"
```

Expected: an `index.json` and exactly one `blobs/<aa>/<hash>.gz`; the index carries `"version": 1`, one key `http://127.0.0.1:8765/report`, `"verdict": "supported"`, a 64-hex `claimsHash`, `"toolVersion": "0.1.0"`, `"localRulesHash": null`, and one read with `"rung": "node"`, `"status": 200`, a `content-type` header and `"pdftotextVersion": null`. The grep must print `0 (good)` - **if `SECRET123` appears anywhere, stop: a credential has been written to a file meant to be committed.**

```bash
cd C:/Users/noaho/testimonium-plan3 && \
rm -rf /tmp/tm-e2e/doc.archive && \
node dist/bin.js check /tmp/tm-e2e/doc.md --no-archive; echo "EXIT=$?"; \
test -e /tmp/tm-e2e/doc.archive && echo "BUG: --no-archive wrote an archive" || echo "no archive written (good)"
```

Expected: `EXIT=0` and `no archive written (good)`.

```bash
cd C:/Users/noaho/testimonium-plan3 && \
node dist/bin.js check /tmp/tm-e2e/doc.md && \
node dist/bin.js check /tmp/tm-e2e/doc.md && \
find /tmp/tm-e2e/doc.archive/blobs -type f | wc -l
```

Expected: `1` - re-running `check` against an unchanged source rewrites the same hash and stores no new blob.

Then stop the server and clean up:

```bash
cd C:/Users/noaho/testimonium-plan3 && \
kill %1 2>/dev/null; rm -rf /tmp/tm-e2e; echo "cleaned"
```

**Paste all four transcripts into the ledger.** If any expectation above does not hold, that is a finding to stop on, not a number to explain away.
...
**The decision procedure, transcribed from 8.3. Condition on `L` first.** `A` is read only to attribute the difference. `R` is the invariant.
...
**Two ordering notes, both recorded as this plan's decisions.** No-baseline is evaluated FIRST even though 8.3 lists the confound row above it, because every confound predicate reads a field off the baseline entry. And a violated `R` invariant degrades to no-baseline rather than being trusted, because trusting it could reach the accusing row from a baseline that never proved anything.
...
describe("the named confounds", () => {
  ...
  it("a confounded citation whose live read 404'd still reports liveGone", () => {
    // 8.3 puts the confound row above the gone row and this plan follows it for
    // the category and the exit contribution. `liveGone` is carried anyway so
    // the report can say it: a claims edit should not make a dead link
    // invisible.
    const o = compareCitation(mk({
      claims: ["a different claim entirely"],
...
export interface CitationOutcome {
  ...
  /** Named confounds, in the report's own words. Empty on every other row. */
  readonly confounds: readonly string[];
  /** Some live read reported 404 or 410. Carried on EVERY outcome, so a
   *  confounded citation can still say the source may be gone; it never
   *  changes the category or the exit contribution. */
...
export function compareCitation(input: CompareInput): CitationOutcome {
  ...
  const liveGone = input.liveStatuses.some(isGoneStatus);
  const base = {
    url: input.url,
    key: input.key,
  ...
  // No baseline. Evaluated FIRST, although 8.3 lists the confound row above it,
  ...
  // Named confounds short-circuit, with an exit contribution of 0. A named
  // confound is NOT pipeline drift and must never be reported as one: pipeline
  // drift means a regression in THIS tool, and in each of these cases what
  // changed is the author's own machine or the author's own data.
  ...
  if (input.liveVerdict === "unreachable") {
    // Gone is its own category, kept apart from transient unreachability -
    // a deleted page is the most common real drift there is, and reporting it
    // as a bare "unreachable now" hides it inside the flaky-network bucket.
    // Transient unreachability is listed, never failed: the page may be
    // perfectly intact and the citation still good.
    return { ...base, category: liveGone ? "gone" : "unreachable", confounds: [] };
  }
...
Expected: `25 passed`, then `TSC CLEAN`.
...
Expected: FAIL naming "L = supported, A = unsupported -> PIPELINE DRIFT, never source drift"; then `25 passed` after the restore.

- [ ] **Step 6: Prove the invariant check discriminates**
...
Expected: FAIL naming "an archived verdict that is not `supported` violates R and degrades to no baseline" (TypeScript may also complain about the unreachable branch, which is fine - it is a mutation, not a commit); then `25 passed`.
...
Expected: `1`, `2`, four `0`s, the previous total **plus 25**, `TSC CLEAN`.
...
Expected: `1`, `2`, four `0`s, no stray temp directories, the previous total **plus 10**, `TSC CLEAN`.
...
```ts
  if (command === "recheck") {
    ...
    for (const [i, o] of report.outcomes.entries()) {
      ...
      // Carried on every row that is not already the gone row: a named confound
      // suppresses the COMPARISON, and it should not also make a dead link
      // invisible.
...
```bash
cd C:/Users/noaho/testimonium-plan3 && \
touch /tmp/tm-drift/DRIFT && \
node dist/bin.js recheck /tmp/tm-drift/doc.md; echo "DRIFT RECHECK EXIT=$?" && \
echo "--- the archive must be untouched ---" && \
node -e 'const j=require("/tmp/tm-drift/doc.archive/index.json"); console.log(Object.keys(j.urls).length, j.urls[Object.keys(j.urls)[0]].verdict)' && \
find /tmp/tm-drift/doc.archive/blobs -type f | wc -l && \
echo "--- the evidence file carries the LIVE arm ---" && \
node -e 'const j=require("/tmp/tm-drift/doc.evidence.json"); console.log(j.results[0].verdict)'
```

Expected: `[1] SOURCE DRIFT`, a `MISS: "spending rose sharply"` line, `1 source drift, 0 pipeline drift, 0 gone since archiving`, `DRIFT RECHECK EXIT=1`; then `1 supported` (the baseline is untouched), `1` (no new blob), and `unsupported` (the evidence file carries the live arm).

```bash
cd C:/Users/noaho/testimonium-plan3 && \
rm -f /tmp/tm-drift/DRIFT && rm -rf /tmp/tm-drift/doc.archive && \
...
kill %1 2>/dev/null; rm -rf /tmp/tm-drift; echo "cleaned"
```

Expected: `[1] no baseline - ... (live: supported)` and `NO BASELINE EXIT=0`; then `unknown flag --fail-on-unrechable` and `TYPO EXIT=2`.

**Paste every transcript into the ledger.** If `DRIFT RECHECK EXIT` is anything other than `1`, or if the archive changed, stop: that is a finding.
...
**gone since** the date it was archived, which exits 0 unless you pass
`--fail-on-gone`. And if you edited that URL's claims, upgraded `pdftotext`, or
changed your own `--rules` file since it was archived, the comparison says so
and compares nothing: those are **named confounds**, and answering "did my
source change?" with an accusation because you edited a claim would be
answering a question with an accusation.
...
**What the archive costs you.** It grows and nothing prunes it: re-archiving a
...
- **New: `check` writes `<doc>.archive/` for every citation that reads
  ...
  it cannot control anything in CI. It grows and nothing prunes it - see the
  README's `Recheck` section for the cost, in full.
- **New command: `testimonium recheck <doc.md>`.** It re-runs each claim
  ...
  0. At the run level 1 dominates 2, deliberately unlike `check`. `recheck`
