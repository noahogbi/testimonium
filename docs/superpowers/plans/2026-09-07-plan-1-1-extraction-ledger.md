...
Worktree: C:\users\noaho\testimonium-plan11 on fix/plan-1-1-extraction, from main @ 7accd06
Plan reviewed by Fable twice: 6 blockers in draft 1, F1-F5 in draft 2, approved at draft 3 (dc93ca4).
...
|---|---|---|---|
| T1 `extract.ts` | T2, T3 | `toText` output changes for named entities | OK — T1 precedes both; T2's sweep baseline is the whole 7accd06 pipeline, so it absorbs T1's change correctly |
...
Task 1: implemented b75a8e5 (code-point entity map; 204 tests, tsc clean). Sweep 0/34 verdicts moved against a REAL 7accd06 checkout built fresh — and sanity-checked non-vacuous: 9/34 fixtures show genuinely different extracted text between builds while no verdict changed.
...
Task 1: review dispatched (sonnet), package review-dc93ca4..b75a8e5.diff
...
Task 1: sweep INDEPENDENTLY reproduced with the reviewer's own script against a fresh 7accd06 worktree — 0/34 verdicts moved AND 9/34 fixtures with genuinely different extracted text, byte-for-byte matching deltas. Fixtures confirmed identical between builds, so the sweep measured the code and not the corpus.
...
Task 1: byte-check independently confirmed — extract.ts fully ASCII; the single U+2019 in the test file verified against 7accd06 as pre-existing, not introduced.
...
Task 1: fix round 1/5 (1 addressed, 0 open; commits b75a8e5..61c8a9a). Re-review independently verified 130 rows / 0 mismatches / 0 duplicates / 0 remaining case asymmetries; confirmed the Szlig claim correct (HTML5 genuinely has no uppercase form); confirmed all four new test cases would fail against the round-1 table; no entity-name prefix or superstring conflicts; &amp; still decodes last.
Task 1: complete (commits dc93ca4..61c8a9a, review clean after 1 fix round)
Task 2: dispatched (sonnet — N5, the veto that asks whether the body is text), BASE=61c8a9a
Task 2: implemented 393cf10 (N5 on Signals + isBlocked; isTextualContentType and looksBinary in signals.ts; reachability reason branch between gone and challenged, pinned by a test). 213/213, tsc + build clean, byte-check clean across all 6 files.
...
Task 2: review dispatched (sonnet), package review-61c8a9a..393cf10.diff
Task 2: review returned spec ✅, quality Approved, 2 Important (both for Task 4 disclosure), 1 Minor. THE ROUTE IS CLOSED, proven with live arxiv bytes: before 61c8a9a `unsupported` with the claim in missed; after, `unreachable` with no evidence and no missed field.
...
Task 2: sweep independently reproduced with the reviewer's own script — 0/34 moved, N5 fired 0/34, 9/34 differing text, max density exactly 0. Discrimination: 8/8 new assertions fail against 61c8a9a.
...
Task 2: complete (commits 61c8a9a..393cf10, review clean)
Task 3: dispatched (sonnet — fixtures for body shapes the corpus has never held), BASE=393cf10
Task 3: implemented 958f299. 216/216, tsc clean. PDF fixture proseVolume 6,221 (1,721 clear of the 4,500 floor, so N5 rejects it and the floor does not — pinned by assertion). Entity fixture 6,394. Corpus 34 -> 36 (25 challenge / 10 document / 1 known-gap). Test files pure ASCII; corpus.json's 6 non-ASCII bytes confirmed pre-existing.
Task 3: review dispatched (sonnet), package review-393cf10..958f299.diff
...
Task 3: complete (commits 393cf10..958f299, 1 Important carried into Task 4)
Task 4: dispatched (sonnet — the documentation task, carrying findings from Tasks 2 and 3), BASE=958f299
Task 4: implemented 4c81090. 231/231 (+10 pdf.test.ts, +5 validateFlags), tsc + build clean, manual live `check example/sample.md` exit 0, evidence file restored.
...
Task 4: review dispatched (sonnet), package review-958f299..4c81090.diff
...
Task 4: README read end-to-end against src/ rather than against the report. "Six inputs" vs "five vetoes" confirmed NOT a contradiction (N3 consumes two inputs). The corrections confirmed real via `git show 958f299:` — the base genuinely said "four vetoes" and omitted N5.
...
Task 4: complete (commits 958f299..4c81090, review clean, 1 Minor parked)
All four tasks complete. Dispatching whole-branch review (opus, 7accd06..4c81090).

## Whole-branch review (opus, 7accd06..4c81090) - verdict: approve with fixes

PURPOSE PROVEN LIVE, not read off the diff: the reviewer fetched the real arxiv PDF (2,215,244 B, content-type application/pdf), decoded it exactly as nodeFetch/curlFetch do, and ran a 7accd06 build and the branch build side by side. 7accd06 -> `unsupported`, naming "Attention Is All You Need" and "The dominant sequence transduction models" as missed. Branch -> `unreachable`, no missed, no evidence. Prose measured 1,037,512 - the plan's figure to the character. Same result with the content-type absent. And the recovery works: check() on the /pdf/ URL -> `supported` via the pdftotext rung with real excerpts from the paper.
Independent sweep against a fresh 7accd06 build from `git archive`: 36 fixtures, 108 comparisons, 0 verdicts moved on the 34 that existed at 7accd06; the only 2 moves are the new PDF fixture, which is the point. 10/36 extract genuinely different text, so the baseline was real. Every calibration figure reproduced exactly. Floor licence intact: 4,500 lies in [1,380, 6,194]. 23 mutations run, 21 killed.
...
- IMPORTANT 3 the spec was never amended - FIX. The plan's own header promises it amends section 6.2; `git log 7accd06..4c81090 -- docs/superpowers/specs/` is EMPTY. The binding authority still says "the four vetoes" and never contains "N5". Plan 1's ledger already recorded one spec/ship divergence; this would be the second.
...
Final fix round: brief at final-fix-brief.md, dispatched (opus), BASE=4c81090

## Final fix round (opus, 4c81090..022b681) - all fourteen items closed, nothing parked

246 tests (from 231), tsc clean, build clean, tree clean. Commits: f9cfe80 (A), 160916a (B+C), fa15d51 (D), d6d65f1 (E+F+N), 0c80c5a (G+H), 8e7b881 (I+J+K+L+M), 5412102 (a correction to its own -j comment), 022b681 (restore example/sample.evidence.json, rewritten by a smoke run).
...
**Controller corrections, committed as 75e4984** - two things the implementer surfaced honestly and left open, both the exact class this branch exists to punish:
...
The implementer reported exactly one item it found and did not fix - spec section 6.3's "consults no status" contradicting N4 - because the brief told it not to restructure the spec and item H's grep did not reach that section. That is now closed by 75e4984. No spec item remains open.
...
Scoped re-review dispatched (sonnet), package review-4c81090..75e4984.diff, 9 commits. Asked to re-run every mutation independently, judge the item-E deviation on its merits, and check documentation against src/ rather than against the report.

## Scoped re-review (sonnet, 4c81090..75e4984) - all fourteen closed, one blocker found

Every item A-N confirmed closed BY RUNNING, not by reading. The re-reviewer independently reproduced: item A's false accusation failing against 4c81090 with the new test (`expected 'unsupported' to be 'unreachable'`); all three item-C mutations killing exactly 3, 1 and 1 named tests with both values unchanged at 0.01 and 65_536; item E's `proven` deletion breaking exactly the repaired test; item N's revert breaking exactly the new test; item J's 55,668-char body reproducing notText:false at density 0.00898; item K's soft-hyphen false-miss on both "cooperation" and "co-operation"; item I's Verge capability loss at 16,449 chars; and every calibration figure from its own build. Test count independently recounted: 246 = 217 literal `it()` + 6 `it.each` tables expanding to 29, reconciled per file.
...
FINAL STATE: 18 commits on 7accd06..0c85e07, 246 tests, tsc clean, build clean, tree clean, zero NULs, all source pure ASCII. Plan 1.1 is complete. Push/PR/merge NOT authorised for this branch - the user's earlier "push and pr, then good to merge and clean up" was given for plan 1's branch specifically. Ask.
