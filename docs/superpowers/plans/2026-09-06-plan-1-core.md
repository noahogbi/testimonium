...
- Produces: `THRESHOLDS: { minProseChars: number; minSlugOverlap: number }` and the two pure helpers `proseVolume(text)` and `slugLabelOverlap(text, url)` that later tasks reuse.
...
**And neither does overlap - measured, not assumed.** A first calibration attempt proved infeasibility rather than failing to search: exhaustive evaluation of 24,915 threshold pairs returned **zero** solutions with the ECB fixture present, and 249 without it. The ECB page scores prose 13,221 and overlap **1.00** - tied for the corpus maximum, against a lowest real document of 0.75. The cause was structural: `slugTitleOverlap` drew content words from the document's own `<title>`, which arrives in the *same response* as the body, so every page contains its own title by construction. All 12 fixtures carrying a title scored exactly 1.00, challenge and document alike.

Two changes followed, and both are already in the code above:

1. **The function is now `slugLabelOverlap`** and draws only from authored inputs - the URL path and the author's footnote label. Never the fetched title.
2. **N4 exists**: HTTP 404 or 410 is a veto. It is the one place status is consulted, and the asymmetry justifies it - a server is not authoritative about presence, but it *is* authoritative when it says a resource does not exist.

**Your first step is therefore to add a `status` field to every entry in `fixtures/corpus.json`.** Get it honestly: re-fetch the 9 document URLs and the 3 real captures to record their actual status codes, and take the constructed battery fixtures' statuses from `fixtures/challenge-battery.mjs`, which declares the status each case was built to represent. **Do not guess a status**, and do not assign 404 to anything merely because it would be convenient - that would be the same curation failure that omitted this fixture in the first place, wearing a different hat.

**If no pair of thresholds satisfies all four assertions, stop and report again.** Do not drop a fixture, lower a margin, weaken an assertion, or retune a metric. A second stop is a second real finding, and it is still cheaper than building the reducer on numbers that do not hold.
...
export function verdict(s: Signals): Verdict {
  ...
  const readProven =
    s.proseChars >= THRESHOLDS.minProseChars && s.slugLabelOverlap >= THRESHOLDS.minSlugOverlap;
