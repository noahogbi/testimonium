...
**Restore it first.** Add that capture to `fixtures/corpus.json` as `kind: "challenge"` (the file is already on disk, unreferenced). It is a genuine error shell and the corpus is worth less without it, because it is the counterexample to the intuition that prose volume alone separates documents from non-documents. **It does not.**

That is why the acceptance test above is written against the **accusation gate** - the conjunction of prose volume AND slug/title overlap - rather than against prose volume alone. Prose volume gets the ECB page nothing; overlap is what must reject it, and the third assertion demands each challenge fixture be clear on at least one dimension. Calibrate BOTH thresholds together.

**If no pair of thresholds satisfies all four assertions, stop and report.** That is the genuine design gap, not a tuning problem, and it is the outcome this gate exists to surface. Do not drop the ECB fixture a second time to make the suite green.
...
import { THRESHOLDS, proseVolume, slugTitleOverlap } from "./thresholds.js";
...
export function computeSignals(input: SignalInput): SignalResult {
  ...
  return {
    ...
    signals: {
      ...
      slugTitleOverlap: slugTitleOverlap(text, input.finalUrl, title, input.sourceLabel ?? ""),
...
Task 4's acceptance test exercises `proseVolume` and `slugTitleOverlap` directly, but spec section 6.3 phrases its requirement **through the verdict** ("must return `unreachable` for 100% of non-documents"). Nothing yet runs the real pipeline over the corpus, so N3's length conjunction and the path and header vetoes are never corpus-exercised. Add `test/classify/corpus-verdict.test.ts`:
...
Also fixed from the lower-priority findings: `dedupeEvidence` now merges **overlapping** windows rather than only byte-identical ones (the motivating bug was two windows centred a few characters apart); evidence records the rung that actually read the document rather than the last one attempted; `slugTitleOverlap` returns 0 rather than a vacuous 1 when there is nothing to correlate, so an opaque URL cannot license an accusation on prose volume alone; the 800-char challenge cap moved into `THRESHOLDS` under the same doctrine as every other verdict-changing number; `RungId` reopened so the bring-your-own-reader mitigation is expressible; `hosts.ts` became a real module rather than a re-export; `check`'s dead `failOn` option removed; `--allow-unclaimed` added and `not_applicable` footnotes printed; and the `ABBREVIATION` regex anchored, since unanchored it suppressed every sentence snap.
