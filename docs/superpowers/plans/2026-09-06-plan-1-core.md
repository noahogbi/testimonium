...
Consequently the corpus must not contain sub-floor documents. Measured for you: the battery's challenge-side maximum is **982 characters** (the padded Turnstile page), so the floor is at least 1,182 and every document fixture must clear floor+200. A first draft of the capture list included a discussion-thread permalink extracting to 513 characters, which made these assertions unsatisfiable and would have triggered the stop-and-report below on a design that was fine.
