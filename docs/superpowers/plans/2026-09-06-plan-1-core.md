...
**Withheld 2026-09-10.** This plan is a historical record and is not rewritten, but the origin repository's absolute path is withheld for publication: the two sentences below that named it now read "the origin repository (private)". A label for a label; no task, step, finding, number or date changed.
...
**Context.** Calibration in Task 4 cannot happen without both halves of the corpus. `fixtures/challenge-battery.mjs` already holds 23 constructed wall bodies but hard-codes a path into the origin repository (private), so it is not yet portable. The document half does not exist and must be captured once, by hand, over the network. **`scripts/capture-fixtures.mjs` is run manually and is never invoked by the test suite.**
...
1. Convert `fixtures/challenge-battery.mjs`: delete its `import` of anything under the origin repository (private) - the file currently hard-codes an absolute path into the origin repo and will not run anywhere else - and write each of its constructed bodies out to `fixtures/challenge/<name>.html`.
...
export interface FetcherOptions {
  /** Merged host rules, bundled plus any the user added via --rules. Omitted
   *  means the bundled snapshot. Without this the override loads, validates,
   *  and changes nothing - which is decoration, not a feature. */
...
import { CHALLENGE_SIGNATURES, CHALLENGE_PATHS } from "../src/rules/challenge.js";
...
describe("check", () => {
  ...
  it("a local rule set CHANGES a verdict - --rules is not decoration", async () => {
    // The functional core had zero coverage: a regression turning --rules back
    // into a no-op would have left the whole suite green. The body carries
    // BOTH the claim and a wall phrase no bundled signature knows, and is
    // short enough for the signature veto's length conjunction to apply - so
    // the local rule is the only thing that can change the outcome.
    const BODY =
      "<html><body>Please solve the puzzle. The committee reported spending rose sharply.</body></html>";
    const fetcher = stub({ node: { rawBody: BODY, status: 200 }, curl: { rawBody: BODY, status: 200 } });
    const claims = ["spending rose sharply"];

    const without = await check("https://e.com/a", claims, { fetcher });
    expect(without.verdict).toBe("supported");

    const rules = {
      signatures: [
        ...CHALLENGE_SIGNATURES,
        { pattern: /solve the puzzle/, lastConfirmed: "2026-09-07", note: "local puzzle wall" },
      ],
      paths: CHALLENGE_PATHS,
      hosts: HOST_RULES,
    };
    const withRules = await check("https://e.com/a", claims, { fetcher, rules });
    expect(withRules.verdict).toBe("unreachable");
    expect(withRules).not.toHaveProperty("evidence");
  });

  it("carries firedRule as provenance, on a non-supported verdict", () => {
    // firedRule is provenance, not renderable evidence, so it may appear on a
    // verdict that carries no excerpt. --explain-fetch reads it.
    const rules = {
      signatures: [
        ...CHALLENGE_SIGNATURES,
        { pattern: /solve the puzzle/, lastConfirmed: "2026-09-07", note: "local puzzle wall" },
    ...
    const BODY = "<html><body>Please solve the puzzle to continue.</body></html>";
    const fetcher = stub({ node: { rawBody: BODY, status: 200 }, curl: { rawBody: BODY, status: 200 } });
    return check("https://e.com/a", ["anything"], { fetcher, rules }).then((r) => {
      expect(r.firedRule?.note).toBe("local puzzle wall");
      expect(r).not.toHaveProperty("evidence");
...
export interface ReachabilityOptions {
  readonly fetcher?: Fetcher;
  /** Same merged rule set check() takes. reachability walks the SAME fetch
   *  ladder, so a user's local signatures must apply here too - otherwise the
   *  preflight reports a host readable that the gate then calls unreachable. */
 ...
 *  only while host rules were unwired and stopped being moot the moment they
 *  reached the fetcher. */
...
- [ ] **Step 6: Fix round 1 (2026-09-07) - hosts wiring and integration coverage**

Two Criticals came back from review of the first pass. Both are the same
failure mode already found and fixed once in this task for `signatures` and
`paths`: a `RuleSet` field that loads, validates, and is then never consulted
by anything. It was not carried across to `hosts`.

**Critical 1 - `RuleSet.hosts` was completely unconsumed.** `hostRuleFor()`
read the module-level `HOST_RULES` constant directly and never saw the merged
set produced by `loadRules()`. Proven by running the built CLI against a real
local HTTP server: a local host rule with `requiresIdentity: true` for the
exact host being fetched produced **no** identity warning, sent the plain
browser UA, and exited 0. Loads, validates, changes nothing. Fix, wired end to
end:

`src/rules/hosts.ts` - `hostRuleFor` takes the list it searches as a parameter
instead of closing over the module constant:
...
`src/fetch/default-fetcher.ts` - `FetcherOptions` gains `hosts`, and
`userAgentFor` passes it through:
...
`src/check.ts` - the default fetcher is built with the merged host list
whenever `opts.rules` is supplied:

```ts
const fetcher = opts.fetcher ?? defaultFetcher(opts.rules ? { hosts: opts.rules.hosts } : {});
```

**Critical 2 - the functional core had no automated coverage.** All 6 of
`test/rules/load.test.ts`'s original tests exercise `loadRules`'s pure merge
logic only. Nothing asserted that `--rules` changes a verdict, or that
`firedRule` reaches a `CitationResult` - so a regression turning `--rules`
back into a no-op would leave the whole suite green. Two integration tests
close that gap, added to `test/check.test.ts`:
...
describe("check with a local RuleSet (Task 15)", () => {
  ...
  // CHALLENGE_SIGNATURES entry knows.
...
**Important 1 - local `hosts` entries now get the same audit discipline as
signatures/paths.** This was moot only while host rules were unwired; it
stops being moot the moment they reach the fetcher (Critical 1, above).
`src/rules/load.ts` gains a `toHostRule` validator, used in `loadRules`'s
`hosts` merge:
...
**Important 2 - `reachability()` now accepts and consults the same `rules`.**
It walks the same ladder `check()` does; a preflight that calls a host
readable while the gate calls it unreachable (because one honored a local
rule and the other didn't) is worse than no preflight. `ReachabilityOptions`
gains `rules?: RuleSet`, threaded into both its `defaultFetcher(...)` call
(mirroring `check.ts`'s Critical 1 fix) and its `computeSignals(...)` call
(mirroring the original Task 15 wiring). In `bin.ts`, `--rules <path>` is
loaded once, up front, **before** the `check`/`reachability` dispatch (it
already was, structurally), and its result is now passed to both:
`reachability(urls, { rules })` alongside the existing `check(c.url, c.claims,
{ sourceLabel: c.label, rules })`.

**Minors, fixed:** `test/rules/load.test.ts` gained a dedicated test for the
`note`-missing branch (the original "requires lastConfirmed and note" test
only ever reached the `lastConfirmed` branch, since it runs first in
`toRule()`), and the first two tests now assert on array/object content, not
just `.length`. Two more tests were added for the new host-rule path
(additive merge + validation), for the same reason Critical 2 called out the
signature/path path: a newly-wired feature with zero test coverage is exactly
the regression risk this fix round exists to close.

- [ ] **Step 7: Verify and commit**

Run: `npx vitest run && npx tsc --noEmit && npm run build`
Expected: full suite green (**150 tests**: 139 pre-Task-15 baseline + 9 in
`test/rules/load.test.ts` + 2 new in `test/check.test.ts`), clean typecheck,
clean build.

```bash
git add src/rules/load.ts src/rules/hosts.ts src/fetch/default-fetcher.ts src/check.ts src/reachability.ts src/bin.ts test/rules/load.test.ts test/check.test.ts
git commit -m "fix: thread local host rules into the fetcher, cover the local-rules path end to end"
```
