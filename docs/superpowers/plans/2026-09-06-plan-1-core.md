...
The ECB capture is now filed as `kind: "challenge"` (Task 3 fix `0650b8b`). It is a genuine error shell and the corpus is worth more with it, because it is the counterexample to the intuition that prose volume alone separates documents from non-documents. **It does not.**
