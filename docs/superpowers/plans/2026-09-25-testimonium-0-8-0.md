...
2. **Percent-encoding and case differences between cited and landed paths** (`/Caf%C3%A9/x` vs `/café`; `/Legislation/BillStatus` vs `/legislation/billstatus.asp`): compared after decode and lowercase; a malformed escape (`%E0%A4%A`) must not throw. Pinned in Task 1.
